...
const LiveHandlePayment = ({ appointmentId, slotNumber, handleFetchSlots, handleCloseForm }) => {
  const {patientId, doctorId} = useParams();
  const navigate = useNavigate();
  const location = useLocation();
  const [appointmentMode, setAppointmentMode] = useState(location.pathname.includes("video") ? "video" : "clinic");
  ...
  return (
    <div className="max-w-screen-md mx-auto p-4">
      {loading ? (
        ...
        <>
          <div className="bg-color text-white p-4 rounded-lg shadow-md mb-4">
            ...
            <p>Date: {new Date(appointmentData.appointmentTime).toLocaleDateString()}</p>
            <p>Time: {new Date(appointmentData.appointmentTime).toLocaleTimeString()}</p>
          ...
          {sessionId && (
            <div className="flex justify-center items-center">
              <button
                ...
                className="bg-green-500 hover:bg-green-700 text-white font-bold md:py-2 px-4 rounded mr-2"
              >
                Confirm Payment
              </button>
              <button
                onClick={() => handlePaymentConfirmation("cancelled")}
                className="bg-red-500 hover:bg-red-700 text-white font-bold md:py-2 px-4 rounded"
              >
                Cancel Payment
              </button>
            </div>
